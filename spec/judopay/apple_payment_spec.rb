...
':"someKey","publicKeyHash":"someKey","transactionId":"someId"}}},"billingAddress":1,"shippingAddress":2}}'
  ...
  it 'should raise an error when bad pk_payment passed' do
    ...
    end).to raise_error(Judopay::ValidationError, Judopay::PkPayment::WRONG_JSON_ERROR_MESSAGE)

    expect(lambda do
      Judopay::ApplePayment.new(:pk_payment => 1)
    end).to raise_error(Judopay::ValidationError, Judopay::PkPayment::WRONG_OBJECT_ERROR_MESSAGE)
